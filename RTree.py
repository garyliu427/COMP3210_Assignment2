# -*- coding: utf-8 -*-
"""
Created on Thu May 27 14:42:03 2021
...
points = []
n = 0
with open("Dataset for R-Tree.txt",'r') as dataset:
...
queries = []
n = 0
with open("100 Queries.txt", 'r') as queriesset:
    for data in queriesset.readlines():
        data = data.split()
        queries.append({
            'x1': int(data[0]),
            'x2': int(data[1]),
        ...
        # print("xn1=",(queries[n]['xn1']), "xn2=",queries[n]['xn2'])
        n = n + 1

def sequentialScanMethod(data, queries):
    seq_start = time.time()
    count = 0
    result = []
    for point in points:
    ...
    seq_end = time.time()
    seq_used_time = seq_end - seq_start

    print("Total Time for sequential queries: ", seq_used_time)
    print("Average Time for every sequential query: ", seq_used_time/100 )
    return seq_used_time/100


def rtreeMethod(data, queries):
    rtree = RTree()
    ...
    results = []
    rtreeQuery_start = time.time()
    with open("query_result.txt",'w') as f:
        for query in queries:
            results.append(rtree.query(rtree.root, query))
        for ele in results:
            f.write("%s\n" % ele)
    rtreeQuery_end=time.time()
    rtree_used_time = rtreeQuery_end - rtreeQuery_start
    
    print("Total time for R-Tree queries:", rtree_used_time )
    print("Average time for R-Tree query",rtree_used_time/100)
    return rtree_used_time/100
...
def main():
    avg_seq_used_time = sequentialScanMethod(data, queries)
    avg_rtree_used_time = rtreeMethod(data, queries)
    print("R-Tree is", avg_seq_used_time/avg_rtree_used_time, "times faster than sequential query")
